 ...
 * 缓存器实现类，磁盘缓存器
 */
public class FLDiskCache implements FLCache {
     ...
     *            缓存大小
     */
    public FLDiskCache(File rootDirectory, int maxCacheSizeInBytes) {
     ...
     */
    public FLDiskCache(File rootDirectory) {
    ...
    public synchronized Entry get(String key) {
        ...
        try {
            ...
            Debug.Log(file.getAbsolutePath(), e.toString());
            remove(key);
            return null;
        } finally {
            if (cis != null) {
    ...
    public synchronized void initialize() {
        if (!mRootDirectory.exists()) {
            if (!mRootDirectory.mkdirs()) {
                Debug.Log("Unable to create cache dir %s",
    ...
    public synchronized void put(String key, Entry entry) {
        ...
        try {
            ...
            if (!success) {
                fos.close();
                Debug.Log("Failed to write header for %s",
        ...
        if (!deleted) {
            Debug.Log("Could not clean up file %s", file.getAbsolutePath());
        }
    }
    ...
    public synchronized void remove(String key) {
        ...
        removeEntry(key);
        if (!deleted) {
            Debug.Log("Could not delete cache entry for key=%s, filename=%s", key, getFilenameForKey(key));
        }
    }
