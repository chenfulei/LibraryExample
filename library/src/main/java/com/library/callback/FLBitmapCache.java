 ...
 */
public class FLBitmapCache extends LinkedHashMap<String , Bitmap> {
    ...
    public FLBitmapCache(int mc, int mp, int mtp){
    ...
    public Bitmap put(String key, Bitmap bm){
        ...
        if(px <= maxPixels){
            ...
            Debug.Log("reject", px + ":" + bm.getWidth() + ":" + bm.getHeight() + ":" + key);
        }
        return old;
    }
    ...
    public Bitmap remove(Object key){
        ...
        Debug.Log("remove pixels", key + ":" + size() + ":" + pixels);
        return old;
    }
    ...
    public boolean removeEldestEntry(Map.Entry<String, Bitmap> eldest) {
        ...
        if(pixels > maxTotalPixels || size() > maxCount){
			if(pixels > maxTotalPixels){
                Debug.Log("evict by max size");
			}else{
                Debug.Log("evict by count :"+ maxCount);
			}
