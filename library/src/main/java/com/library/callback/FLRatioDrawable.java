 ...
 */
public class FLRatioDrawable extends BitmapDrawable{
    ...
    public FLRatioDrawable(Resources res, Bitmap bm, ImageView iv, float ratio, float anchor){
    ...
    private int targetHeight(int dw, int dh, int vw){
        float r = ratio;
        if(ratio == FLConstants.RATIO_PRESERVE){
            r = (float) dh / (float) dw;
        }
        return (int) (vw * r);
    }
    ...
    private float getYOffset(int vwidth, int vheight){
        if(anchor != FLConstants.ANCHOR_DYNAMIC){
            return (1 - anchor) / 2;
        }
