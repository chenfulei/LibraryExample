 ...
 */
public abstract class FLPullToRefreshBase<T extends View> extends LinearLayout implements FLPullToRefresh<T>{
    ...
    // 下拉刷新和加载更多的监听器
    private OnFLRefreshListener<T> mRefreshListener;
    // 下拉刷新的布局
    private FLLoadingLayout mHeaderLayout;
    // 上拉加载更多的布局
    private FLLoadingLayout mFooterLayout;
    // HeaderView的高度
    ...
    // 下拉的状态 */
    private FLLoading.FLLoadState mPullDownState = FLLoading.FLLoadState.NONE;
    // 上拉的状态 */
    private FLLoading.FLLoadState mPullUpState = FLLoading.FLLoadState.NONE;
    // 可以下拉刷新的View
    T mRefreshableView;
    // 平滑滚动的Runnable
     ...
     * @param <V>
     */
    public interface OnFLRefreshListener<V extends View>{
         ...
         * @param refreshView
         */
        void onPullDownToRefresh(FLPullToRefreshBase<V> refreshView);

        /**
         * 加载更多时会被调用或上拉时调用
         * @param refreshView
         */
        void onPullUpToRefresh(FLPullToRefreshBase<V> refreshView);
    }

    /**************构造方法*******************/
    public FLPullToRefreshBase(Context context){
        super(context);
        init(context , null);
    }

    public FLPullToRefreshBase(Context context , AttributeSet attrs){
        super(context, attrs);
        init(context, attrs);
    }

    public FLPullToRefreshBase(Context context, AttributeSet attrs, int defStyle){
            ...
            case MotionEvent.ACTION_UP:
                if (mIsHandledTouchEvent) {
                    ...
                    if (isReadyForPullDown()) {
                        // 调用刷新
                        if (mPullRefreshEnabled && (mPullDownState == FLLoading.FLLoadState.RELEASE_TO_REFRESH)) {
                            startRefreshing();
                            handled = true;
                        }
                        resetHeaderLayout();
                        ...
                        // 加载更多
                        if (isPullLoadEnabled() && (mPullUpState == FLLoading.FLLoadState.RELEASE_TO_REFRESH)) {
                            startLoading();
                            handled = true;
                        }
                        resetFooterLayout();
    ...
    @Override
    public void setOnHCRefreshListener(OnFLRefreshListener<T> refreshListener) {
    ...
    public void onPullDownRefreshComplete() {
        if (isPullRefreshing()) {
            mPullDownState = FLLoading.FLLoadState.RESET;
            onStateChanged(FLLoading.FLLoadState.RESET, true);
            ...
            postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    mHeaderLayout.setState(FLLoading.FLLoadState.RESET);
                }
    ...
    public void onPullUpRefreshComplete() {
        if (isPullLoading()) {
            mPullUpState = FLLoading.FLLoadState.RESET;
            onStateChanged(FLLoading.FLLoadState.RESET, false);

            postDelayed(new Runnable() {
                @Override
                public void run() {
                    setInterceptTouchEventEnabled(true);
                    mFooterLayout.setState(FLLoading.FLLoadState.RESET);
                }
            }, getSmoothScrollDuration());
     ...
     */
    protected FLLoadingLayout createHeaderLoadingLayout(Context context, AttributeSet attrs) {
     ...
     * @return LoadingLayout对象
     */
    protected FLLoadingLayout createFooterLoadingLayout(Context context, AttributeSet attrs) {
    ...
    protected void addHeaderAndFooter(Context context) {
        ...
        final FLLoadingLayout headerLayout = mHeaderLayout;
        final FLLoadingLayout footerLayout = mFooterLayout;
    ...
    protected void pullHeaderLayout(float delta) {
        ...
        if (isPullRefreshEnabled() && !isPullRefreshing()) {
            if (scrollY > mHeaderHeight) {
                mPullDownState = FLLoading.FLLoadState.RELEASE_TO_REFRESH;
            } else {
                mPullDownState = FLLoading.FLLoadState.PULL_TO_REFRESH;
            }
    ...
    protected void pullFooterLayout(float delta) {
        ...
        if (isPullLoadEnabled() && !isPullLoading()) {
            if (scrollY > mFooterHeight) {
                mPullUpState = FLLoading.FLLoadState.RELEASE_TO_REFRESH;
            } else {
                mPullUpState = FLLoading.FLLoadState.PULL_TO_REFRESH;
            }
    ...
    protected boolean isPullRefreshing() {
        return (mPullDownState == FLLoading.FLLoadState.REFRESHING);
    }
    ...
    protected boolean isPullLoading() {
        return (mPullUpState == FLLoading.FLLoadState.REFRESHING);
    }
    ...
    protected void startRefreshing() {
        ...
        mPullDownState = FLLoading.FLLoadState.REFRESHING;
        onStateChanged(FLLoading.FLLoadState.REFRESHING, true);

        if (null != mHeaderLayout) {
            mHeaderLayout.setState(FLLoading.FLLoadState.REFRESHING);
        }
        ...
        if (null != mRefreshListener) {
            ...
            postDelayed(new Runnable() {
                @Override
                public void run() {
                    mRefreshListener.onPullDownToRefresh(FLPullToRefreshBase.this);
                }
            }, getSmoothScrollDuration());
        }
    }
    ...
    protected void startLoading() {
        // 如果正在加载
        if (isPullLoading()) {
            Debug.Log("" + isPullLoading());
            return;
        }

        mPullUpState = FLLoading.FLLoadState.REFRESHING;
        onStateChanged(FLLoading.FLLoadState.REFRESHING, false);

        if (null != mFooterLayout) {
            mFooterLayout.setState(FLLoading.FLLoadState.REFRESHING);
        }
        ...
        if (null != mRefreshListener) {
            ...
            postDelayed(new Runnable() {
                @Override
                public void run() {
                    mRefreshListener.onPullUpToRefresh(FLPullToRefreshBase.this);
                }
            }, getSmoothScrollDuration());
        }else{
            Debug.LogE("mRefreshListener is null");
        }
    }
     ...
     */
    protected void onStateChanged(FLLoading.FLLoadState state, boolean isPullDown) {
    ...
    final class SmoothScrollRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (mContinueRunning && mScrollToY != mCurrentY) {
                FLPullToRefreshBase.this.postDelayed(this, 16);// SUPPRESS CHECKSTYLE
            }
        }
