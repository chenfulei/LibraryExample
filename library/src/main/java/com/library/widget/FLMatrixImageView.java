 ...
 * 
 * @author chen_fulei
 *         注：当该ImageView在其他ViewGroup中，如常用的ViewPager中时，Move事件会和ViewGroup事件冲突
 *         。在划屏进行ViewPager的Item切换时
 *         ，Viewpager将会通过onInterceptTouchEvent方法拦截掉，返回给ImageView一个Cancel事件
 *         ，这种情况下需要重写ViewGroup的onInterceptTouchEvent方法。
 */
public class FLMatrixImageView extends ImageView {
...
//	public HCMatrixImageView(Context context, AttributeSet attrs,
...
//	}
	...
	public FLMatrixImageView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		// TODO Auto-generated constructor stub
		init();
	}

	public FLMatrixImageView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public FLMatrixImageView(Context context) {
		super(context, null);
		init();
	}
	public MatrixTouchListener getTouchListener(){
		if(null == mListener){
			return new MatrixTouchListener();
		}
		...
		if (getWidth() == 0) {
			...
			vto.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
				public boolean onPreDraw() {
					initData();
					// 赋值结束后，移除该监听函数
					FLMatrixImageView.this.getViewTreeObserver()
