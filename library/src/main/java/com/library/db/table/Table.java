...
public class Table {
    ...
    private Table(FLDbUtils db, Class<?> entityType) {
        this.db = db;
    ...
    public static synchronized Table get(FLDbUtils db, Class<?> entityType) {
    ...
    public static synchronized void remove(FLDbUtils db, Class<?> entityType) {
        String tableKey = db.getDaoConfig().getDbName() + "#" + entityType.getName();
        tableMap.remove(tableKey);
    }

    public static synchronized void remove(FLDbUtils db, String tableName) {
        if (tableMap.size() > 0) {
            String key = null;
